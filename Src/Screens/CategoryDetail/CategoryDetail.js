...
const CategoryDetail = props => {
  ...
  useEffect(() => {
    getChangecategryproduct(afterelement, id);
    setTimeout(() => {
  ...
  const getChangecategryproduct = async (afterElement, id = 1) => {
    try {
      // setLoading(true);
      await _axiosGetAPI(
  ...
  const getcategryproduct = async (ids) => {
    if (hasmore) {
      try {
  ...
  const searchproduct = async name => {
    try {
      await _axiosGetAPI(
        `store/categories/${id}/products?limit=20&afterElement=0&search=name%3D${name}`,
      )
        .then(async response => {
          setproductlist(response?.data?.data?.products);
        })
      ...
      <View style={{ marginHorizontal: wp(5), flex: 1, paddingBottom: hp(3), backgroundColor: 'transparent' }}>
        {productlist.length > 0 ? (
          <FlatList
            ...
            // onResponderEnd={getcategryproduct(afterelement)}
